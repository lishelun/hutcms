...
layui.define(['laytpl', 'layer'], function(exports){
  ...
  service.interceptors.request.use(
      (config) => {
          ...
          // 在发送请求之前做些什么 token
          if (token) {
              (config.headers).common[setter.request.tokenName] = token;
          }
          return config;
      },
      (error) => {
  ...
  service.interceptors.response.use(
      (response) => {
          ...
          if (res.code && res.code !== setter.response.statusCode.ok) {
              // `token` 过期或者账号已在别处登录
              if (res.code === setter.response.statusCode.logout) {
                 view.exit();
              }else if(res.code===setter.response.statusCode.auth){
                 layer.msg("权限不足");
              }
      ...
      (error) => {
          debug = function(){
            return setter.debug
              ? '<br><cite>URL：</cite>' + error.request.responseURL
            : '';
          };
          ...
          let errorText = [
            '请求异常，请重试<br><cite>错误信息：</cite>'+ error.response.status+ ' '+errorMessage
            ,debug()
          ].join('');
  ...
  window.request=view.request=function(config){
  ...
  Class.prototype.render = function(views, params){
    ...
    $.ajax({
      ...
      ,error: function(e){
        ...
        if(e.status === 404){
          that.render('tpl/empty');
        } else {
          that.render('tpl/fail');
        }
...
/*          view.req({
          ...
          view.request({
            method: dataElem.attr('lay-type') || 'get',
            url:url,
            data:data,
            responseType:'json'
          }).then((res)=>{
